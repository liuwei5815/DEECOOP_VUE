<!----------------------------dictionaryManage 用户自定义修改  注意要与最下边对应的样式名称保持一致-------------------------------->
<template>
    ...
    <!--<el-row :gutter="10" style="box-sizing: border-box;position: relative;overflow: hidden;transition: height 0.4s;-webkit-transition: height 0.4s;-moz-transition: height 0.4s;" :style="{height: dynaHeight+'px'}">-->
      <!--<div style="position: absolute;">-->
        <!--<el-form :inline="true" :model="tempQuery" class="form-inline">-->
          ...
          <!--<el-col :span="9" style="max-width: 260px">-->
    ...
    <el-row style="border: 1px solid #E4E4E4;border-bottom: none;margin-top: 10px;transition: height 0.4s;-webkit-transition: height 0.4s;-moz-transition: height 0.4s;" :style="{height: tableHeight+'px'}">
      ...
      <el-col :span="4" class="treeStyle">
        <!--<div style="width: 90%;margin: 5px auto">-->
          <!--<el-input :placeholder="'字典名称'" v-model="filterTextDept" clearable>-->
            <!--<i slot="suffix" class="el-input__icon el-icon-search"></i>-->
          <!--</el-input>-->
        <!--</div>-->
        <!--<div style="padding:5px 0px 10px 20px">-->
          <!--<el-checkbox v-model="showDisableDept">显示封存字典</el-checkbox>-->
        <!--</div>-->
...
<script>
  ...
  //*******************************************************用户自定义修改 start ***************************************************************************
  import {
    listTreeDictionary,//树表查询
    listPageDictionary, //分页列表查询查询
    updateDisableDictionary, //启用禁用功能
    addDictionary, //新增功能
    exportExcelDictionarys,//Excel导出
    updateDictionary, //修改功能
    deleteDictionarys //批量删除功能
  } from '@/api/dictionary'
  //按钮权限
  ...
  export default {
    ...
    methods: {
      ...
      changeState(item, index) {
        ...
        //ajax调用启用禁用后台方法
        //*******************************************************用户自定义修改 start ***************************************************************************
        updateDisableDictionary(item.id, item.isdisable )
        //*******************************************************用户自定义修改 end ***************************************************************************
          .then(res => {
            // console.log(res)
            // that.$loading().close()
      ...
      addNew() {
        ...
        if(this.userInfo.userType!='6839818aecfc41be8f367e62502dfde4'&&this.selectTree.pid=='root'){
          this.$message({
            type: 'info',
            message: '只有超级管理员才能新增数据字典一级节点!'
          });
          return
        }
        //政治面貌：'015cecdb7fdd450c8a21c7c97d406aa4' 货品类型：'a39ac4c1e02e45788eb03a52a5e9a972' 客户类型：'a50dcf66b14a440282eed9e26c1d9482'  供应商类型：'d28640b12a454246b172c49a604a89f5'  设备类型：'7d24edc83dcf4619b618bf0b0eba2851' 计量单位：'0ae6e79890db490585e13f34bf00ea4b' 退单原因：'a66d180a1a3346a1a3f4f224baf4ada8'
        if(this.selectTree.layer=='2'&&(this.selectTree.pid=='015cecdb7fdd450c8a21c7c97d406aa4'||'a39ac4c1e02e45788eb03a52a5e9a972'||'a50dcf66b14a440282eed9e26c1d9482'||'d28640b12a454246b172c49a604a89f5'||'7d24edc83dcf4619b618bf0b0eba2851'||'0ae6e79890db490585e13f34bf00ea4b'||'a66d180a1a3346a1a3f4f224baf4ada8')){
          this.$message({
            type: 'info',
            message: '该数据字典类型不支持新增三级节点!'
          });
          return
        }
      ...
      exportExcel() {
        ...
        exportExcelDictionarys(ids)
          .then(res => {
            // console.log(res)
            if (!res) {
              return
            }
            downLoad(res,"数据字典导出.xls")
            // that.$message({
            //   title: '成功',
            //   message: '导出成功',
            //   type: 'success',
