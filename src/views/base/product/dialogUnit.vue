<template>
  <el-dialog v-el-drag-dialog  :title="title" :visible.sync="isVisible" @close="dialogClose" append-to-body width="1000px">
    ...
    <div v-loading="dialogTopLoading">
        ...
        <div style="height: 50px;box-sizing: border-box;padding: 7px 20px 7px 7px;" v-if="configs.topBarBtnList && configs.topBarBtnList.length">
        ...
        <div style="padding: 7px;">
          <base-table :data="tableData" :colConfigs="configs.tableConfigs" :showCheckBox="configs.showCheckBox"
                      :showIndex="configs.showIndex" @databridge="dataBridge"  :isSingle="configs.isSingle" :isSortable="false" :pageData="pageData" :currentpage.sync="pageNo"></base-table>
        </div>
...
<script>
  ...
  import { listPageProductUnits,updateDisableProductUnit,deleteProductUnit,updateIsDefaultProductUnit } from '@/api/productUnit'
  import dialogUnitAdd from './dialogUnitAdd'
  import dialogUnitEdit from './dialogUnitEdit'
  ...
  export default {
    ...
    methods: {
      ...
      editBtns(item, index) {
        ...
        let pnFormula = temp.npFormula;
        let npFormula = temp.pnFormula;

        temp.pnFormula = pnFormula;
        temp.npFormula = npFormula;

        this.dialogData.temp = temp;
        this.dialogData.temp.unitName = this.initData.temp.unitName
        this.dialogData.tempLable = this.tempLable
        this.dialogData.title = '编辑'
        this.openDialogName = 'dialogUnitEdit'
      ...
      changeState(item, index) {
        const that = this
        updateDisableProductUnit(item.id,item.isdisable )
          .then(res => {
          }).catch(err => {
            console.log(err)
          })
      ...
      initTable(isClosedLoading){
        ...
        listPageProductUnits({
          //参数
          ...that.tempQuery,
          productId:this.initData.temp.id,
          pageNo:that.pageNo
        }).then(res => {
      ...
      addNew() {
        ...
        this.dialogData.tempLable = this.initData.tempLable
        this.dialogData.title = '新增计价单位'
        this.openDialogName = 'dialogUnitAdd'
        this.showDialog = true
        this.visibleState = true
      },
      ...
      changeIsdefault(item, index) {
        ...
        // //ajax调用启用禁用后台方法
        // console.log(item)
        updateIsDefaultProductUnit(item.id,item.isdefault,item.productId)
          .then(res => {
            that.refreshTable()
          }).catch(err => {
          console.log(err)
      ...
      refreshTable() {
        const that = this
        that.$axios.all([
          listPageProductUnits({  ...that.tempQuery,productId:this.initData.temp.id,pageNo: that.pageNo })
