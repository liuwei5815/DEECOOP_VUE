    ...
    methods: {
      ...
      initTable(isClosedLoading){
        ...
        listPageBoms({
          ...
          ...that.tempQuery,
          modelCode:'BomList',
          queryStr:' bom.id is not null ',
          prodId:this.initData.temp.prodId,
          pageNo:that.pageNo,
          orderStr:'bom.code asc'
        }).then(res => {
      ...
      refreshTable() {
        const that = this
        that.$axios.all([
          listPageBoms({  ...that.tempQuery,prodId:this.initData.temp.prodId,modelCode:'BomList',pageNo: that.pageNo,orderStr:'bom.code asc' })
