...
<script>
  ...
  import {addBomTrees} from '@/api/bomTree'
  export default {
    components: {
      customTransferTable,
      // transferTable,
      ...
      return {
        ...
        aboveTableConfig: {
          ...
          apiObj: {
            ...
            params: {fieldCode: 'code,name,spec,unitName,genreName,remark',isNeedNotInGenreSpare:'true',notInProductIds: "'"+this.initData.selectTree.id+"'"},
          },
          //自定义列
          customColumn: [//自定义列
    ...
    methods: {
        ...
        if(list){
          if(list.length>0){
            for(let i=0;i<list.length;i++){
              let inDetail = {prodId:"", ratio:"",bomId:"",parentProdId:"",pathId:"",bomTreeId:"",layer:""};
              inDetail.prodId = list[i].id;
              if(list[i].ratio){
                inDetail.ratio = list[i].ratio;
              }else{
                this.$message({
                  type: 'error',
                  message: '请输入用料比例！'
              ...
              inDetail.bomId = this.initData.selectTree.bomId;
              inDetail.parentProdId = this.initData.selectTree.id;
              inDetail.pathId = this.initData.selectTree.pathId;
              inDetail.bomTreeId = this.initData.selectTree.bomTreeId;
              inDetail.layer = this.initData.selectTree.layer;
              inDetailArray.push(inDetail);
            }

            let dtlJsonStr = JSON.stringify(inDetailArray);
            that.dialogLoading = true
            //*******************************************************用户自定义修改 start ***************************************************************************
            addBomTrees({dtlJsonStr:dtlJsonStr})
            //*******************************************************用户自定义修改 end ***************************************************************************
              .then(res => {
      ...
      initTree() {
        ...
        request({
          ...
          method: 'post',
          data: {dictionaryKey: "productGenre",isNotNeedName:'true',nameValue:'备件'}
        }).then(res => {
          that.loadTree(res)
        }).catch(err => {
          console.log(err)
