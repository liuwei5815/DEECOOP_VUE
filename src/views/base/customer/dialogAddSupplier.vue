<template>
  <el-dialog v-el-drag-dialog :title="initData.title?initData.title:'新增'" :visible.sync="isVisible" :append-to-body="inDialog" :close-on-click-modal="onModal" :close-on-press-escape="onEscape"
             :fullscreen="fullscreen" @open="dialogOpen" @close="dialogClose" width="400px">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //省直辖市-下拉列表值改变事件
      provinceSelect(data,option,optionNames){
        //this.temp.province = data[0]
        //console.log(option);
        //console.log(optionNames);
        if(data&&data.length>0){
          this.temp['province'] = option.id
      ...
      dialogOpen(){
        const genre = '49c0a7ebcb4c4175bd5195837a6a9a13'
        this.temp.genre = genre
            ...
            addCustomer(that.temp)
              .then(res => {
                ...
                if (res.code === 0) {
                  ...
                  that.dialogClose({resState:'ok',supplierId:res.id,supplierName:this.temp.name})
                }
              }).catch(err => {
              console.log(err)
              that.dialogLoading = false
