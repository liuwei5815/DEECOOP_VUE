<template>
  <el-dialog v-el-drag-dialog :title="initData.title?initData.title:'编辑'" :visible.sync="isVisible" :append-to-body="inDialog"
             :close-on-click-modal="onModal" :close-on-press-escape="onEscape" :fullscreen="fullscreen" @open="dialogOpen" @close="dialogClose" :width="dialogWidth">
...
<script>
  import {updateCustomerInvoice} from '@/api/customerInvoice'
      ...
      save() {
        ...
        that.$refs['submitForm'].validate((valid) => {
          if (valid) {
            ...
            updateCustomerInvoice(that.temp).then(res => {
