  ...
  <el-dialog v-el-drag-dialog :title="initData.title?initData.title:''" :visible.sync="isVisible" :append-to-body="inDialog"
    ...
    <div style="padding: 10px;">
      <!--TABLE栏开始-->
            ...
            <!--列表界面按钮动态加载-->
            <el-row class="tableTopBar" v-if="configs.topBarBtnList && configs.topBarBtnList.length">
...
<script >
  ...
  import {
    listPageCustomeAddress,       //分页列表查询查询
    updateDefaultCustomerAddress, //设置默认值
    deleteCustomerAddress,        //批量删除功能
  } from '@/api/customeAddress'
  ...
  export default {
    ...
    created() {
      ...
      //console.log("addressList created() jsonStr: " + jsonStr);
      ...
      that.initTable(customerId, true)
    },
    ...
    methods: {
      ...
      changeState(item, index) {
        //this.temp (vmes_customer_address:客户供应商地址表) 行数据

        const that = this
        //ajax调用启用禁用后台方法
        let addressId = item.id;
        let customerId = this.temp.id;
        updateDefaultCustomerAddress(addressId, customerId, item.isdefault)
          .then(res => {
            this.refreshTable();
          }).catch(err => {
            if ("0" == item.isdefault) {
      ...
      editBtns(item, index) {
        ...
        //var jsonStr = JSON.stringify(this.temp);
        //console.log("addressList addNew() jsonStr: " + jsonStr);

        //this.temp (vmes_customer_address:客户供应商地址表) 行数据
        this.dialogData.temp = this.temp
        this.temp.customerId = this.temp.customerId;
        this.dialogData.tempLable = this.tempLable
        this.dialogData.title = '编辑'
        this.openDialogName = 'addressEdit'
      ...
      addNew() {
        ...
        //this.temp (vmes_customer:客户供应商表) 行数据

        //客户供应商地址(添加)界面
        this.dialogData.temp = {name:null, mobile:null, phone:null, email:null, address:null, remark:null, isdefault:null, customerId:null};
        this.dialogData.temp.customerId = this.temp.id;
        this.temp.customerId = this.temp.id;
        this.dialogData.tempLable = this.tempLable
        this.dialogData.title = '新增'
        this.openDialogName = 'addressAdd'
        this.showDialog = true
        this.visible = true
      },
