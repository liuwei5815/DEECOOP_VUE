  ...
  <div class="cascadeTable">
    ...
    <el-row
      ...
      <custom-base-table
        ...
        :showIndex="true"
        :showCheckBox="true"
        :showTableSet="showTableSet"
        :tableSetMode="tableSetMode"
        :customColumn="customColumn"
        :apiObj="setApiObj"
        :mergeTitle="mergeTitle"
        :mergeColumn="mergeColumn"
        :mergeRow="mergeRow"
        :componentId="'customBaseTable'"
        ...
        @callmethod="callMethod"></custom-base-table>
    </el-row>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        postfix: {
          ...
          productUnitName: {
            expression: '#self@productName#<2',//表达式，取行数据进行运算，最终结果为true或false
            //三种类型：icon|text|rowData
            //text配置如下
            type: 'text',
            //后缀的内容配置，是一个数组，如果只有一项，则会在表达式结果为true的时候显示配置；如果有两项，则在表达式为true时显示第一项配置，为false时显示第二项配置
        ...
        //列控制器
        showTableSet: true,
        tableSetMode: 'inline',//列控制的显示模式，还可设置为outside，inline表示控制器在title内部，默认inline，outside表示控制器在表格外部的左上角
    ...
    //vue实例化后调用
    created() {
      //this.tableData = this.getListPage(testData,this.customColumn)
      //this.tableData = testData
      //this.tempItem = JSON.parse(JSON.stringify(testData.result.varList[1]))
      // const tempItem2 = JSON.parse(JSON.stringify(testData.result.varList[1]))
      // tempItem.id = '1'
      // this.tempList.push(tempItem)
      // setTimeout(_=>{
      //   this.varList = this.tempList
      //   setTimeout(_=>{
      //     tempItem2.id = '2'
      //     this.tempList.push(tempItem2)
      //     this.varList = this.tempList
      //     setTimeout(_=>{
      //       this.varList[0].progress = 110
      //     },5000)
      //   },10000)
      // },5000)

      // setTimeout(_=>{
      //   this.customSummary.data.orderAmount = 5
      //   this.customSummary.data.invoice = 5
      // },10000)
    },
    ...
    methods: {
      ...
      pushData(){
        // this.pushIndex += 1
        // const tempItem = JSON.parse(JSON.stringify(this.tempItem))
        // tempItem.id = this.pushIndex
        // this.varList.push(tempItem)
        // console.log(this.varList)

        this.$refs.customBaseTable.updateTable()
      },
      databridge(list){
        console.log(list)
      },
