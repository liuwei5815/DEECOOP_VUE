import { createUUID, deepClone } from '@/utils/common'
 ...
 */
let getCommonStyle = function (styleObj, baseWidth,baseHeight,viewWidth) {
  // console.log(styleObj)
  //let needUnitStr = ['paddingTop', 'paddingLeft', 'paddingRight', 'paddingBottom', 'marginTop', 'marginLeft', 'marginRight', 'marginBottom', 'borderWidth','fontSize', 'borderRadius', 'letterSpacing']
  let needWidthRatio = ['width', 'left']
  let needHeightRatio = ['height','top']
  let needRatio = ['fontSize']
  let tempObj = deepClone(styleObj)
  let style ={}

  for (let key in tempObj){
    if(needWidthRatio.includes(key)){
      style[key] = `${(tempObj[key]/baseWidth)*100}%`
    }else if(needHeightRatio.includes(key)){
      style[key] = `${(tempObj[key]/baseHeight)*100}%`
    }else if(needRatio.includes(key)){
      style[key] = `${tempObj[key]*((viewWidth || baseWidth)/baseWidth)}px`
    }else{
