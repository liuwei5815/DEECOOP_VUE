<template>
  <div
          ...
          tabindex="0"
          @keyup.esc="escUserSelectComponents"
          @keyup.46="deleteUserSelectComponents">
    ...
    created(){
        ...
        if(this.showOnly) return
				...
				if (point.length === 2) {
					...
					// !#zh 上下点，宽度固定在中间
					if (hasT || hasB) {
						newLeft = (width / 2) - ((point === 't' || point === 'b')?5:0);
						newTop = hasT ? 0 : height
					}
					// !#zh 左右点，高度固定在中间
					if (hasL || hasR) {
						newLeft = hasL ? 0 : width
						newTop = (height / 2) - ((point === 'l' || point === 'r')?5:0);
					}
				}
				const style = {
      ...
      escUserSelectComponents(){
			  if(this.showOnly) return
			  if(this.active){
			    this.$emit('reset-active',{uuid:this.uuid,type: this.data.type})
        }
      },
      deleteUserSelectComponents(){
        if(this.showOnly) return
        if(this.active){
          this.$emit('delete-active',this.uuid)
        }
      },
		}
	}
